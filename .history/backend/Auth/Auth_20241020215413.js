...
router.post('/login',async(req,res)=>{
    ...
    router.get('/getuser/:id', fetchuser, async (req, res) => {
        try {
            const userId = req.params.id; // Get the userId from the authenticated user's request
            const user = await User.findById(userId).select('ameNumber'); // Find user by ID and include ameNumber
