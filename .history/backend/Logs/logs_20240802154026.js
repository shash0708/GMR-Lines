...
router.post('/export', async (req, res) => {
  ...
  try {
    ...
    // // Add the heading row
    // const headingRow = worksheet.addRow(['', '', '', '', '', 'AME Workbook', '', '', '', '', '']);
    // headingRow.font = { name: 'Arial', bold: true, size: 14 };
    // headingRow.alignment = { horizontal: 'center', vertical: 'middle' };
    // headingRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFF00' } }; // Yellow fill color
    
    // Merge cells for the heading
    worksheet.mergeCells('A1:L1');

    // Add the column headers in the second row
    worksheet.addRow(['ID', 'ToA', 'Reg', 'ATA', 'Wo', 'Mt', 'TOM', 'TOA', 'C', 'DU', 'Supervisor']);

    // Add rows to the worksheet (starting from row 3)
    logs.forEach(log => {
      worksheet.addRow({
        Id: log.Id,
        ToA: log.ToA,
