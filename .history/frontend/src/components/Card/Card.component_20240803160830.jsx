...
const Cards = () => {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/logs/export', selectedIds, {
        headers: {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const selectedIds = selectedLogs.map(log => log.Id);
      const response = await axios.post('http://localhost:5000/logs/exportpdf', selectedIds, {
        headers: {
            'Content-Type': 'application/json',
            "auth-token": localStorage.getItem('token')
        },
      ...
      <CardContainer>
        {filteredLogs.map((log) => (
          <div className='card-item' key={log.Id}>
            ...
            />
            <Card date={new Date(log.createdAt).toLocaleDateString()} id={log.Id} />
          </div>
        ))}
      </CardContainer>
