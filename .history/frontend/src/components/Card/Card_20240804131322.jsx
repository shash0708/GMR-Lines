...
const Card = ({ date, id,ToA }) => {
  ...
  useEffect(() => {
    const fetchLog = async () => {
      try {
        const response = await fetch(`https://gmr-lines.onrender.com/getlogs/logs/${id}`,{
          method: 'GET',
          headers: {
  ...
  const handleUpdate = async () => {
    try {
      const response = await fetch(`https://gmr-lines.onrender.com/update/updatenotes/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          "auth-token": localStorage.getItem('token')
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(`http://localhost:5000/delete/deleteLog/${id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "auth-token": localStorage.getItem('token')
