...
const LogForm = ({ user }) => {
  ...
  const [formState, setFormState] = useState({
    Id: '',
    dateTime: null, // Single dateTime field for MobileDateTimePicker
    Location: '',
    ACTtype: '',
    ACRegNo: '',
    TOM: '',
  ...
  const handleDateChange = (newValue) => {
    const formattedDateTime = dayjs(newValue).format('DD-MM-YYYY HH:mm'); // Format dateTime
    setFormState((prevState) => ({
      ...prevState,
      ...
      <form onSubmit={handleSubmit}>
        <Grid container spacing={2}>
          {Object.keys(formState).map((field) => (
            field !== 'dateTime' && ( // Skip the dateTime field for now
              field === 'TOM' || field === 'CPU' ? (
