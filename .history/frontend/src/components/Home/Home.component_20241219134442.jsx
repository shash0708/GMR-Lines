...
const CardContainer = styled.div`
  ...
  padding: 10px;
  background-color: ${({ darkMode }) => (darkMode ? '#212020' : '#f5f5f5')}; /* Dark or light background */
`;
...
const Card = styled.div`
  ...
  border-radius: 8px;
  border: 1px solid ${({ darkMode }) => (darkMode ? '#333' : 'grey')};
  ...
  return (
    <div className={`Home ${darkMode ? 'dark-mode' : ''}`}>
      ...
      <CardContainer darkMode={darkMode}>
        {cardData.map(card => (
          <Card darkMode={darkMode} key={card.Id}>
            <div>
            <IconWrapper darkMode={darkMode}>{card.icon}</IconWrapper> {/* Wrap icon in styled component */}
