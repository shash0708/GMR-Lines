...
const Profile = () => {
  ...
  const handleSettingsClick = () => {
    setSettingsMenuOpen(prev => !prev); // Toggle settings menu
  };

  const handleSignOut = () => {
    localStorage.removeItem('token'); // Remove the token
    localStorage.removeItem('ameNumber'); // Optionally clear AME
  ...
  return (
    <div>
      ...
      <IoSettingsOutline className={`settings ${settingsMenuOpen ? 'rotate' : ''}`} onClick={handleSettingsClick} />
      <h1>Profile</h1>
      </div>
      ...
      {settingsMenuOpen && (
        <div className="settings-menu">
          ...
          {userData ? (
            ...
            <button onClick={() => navigate('/signin')} className="settings-button">Sign In</button>
          )}
        </div>
      )}
