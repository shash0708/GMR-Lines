...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (AME) {
        try {
          const response = await axios.get(`http://localhost:5000/api/getuser/${AME}`, {
            headers: {
              'auth-token': localStorage.getItem('token'), 
            },
          });
          setUserData(response.data); 
          setProfileImage(response.data.profileImage); // Assuming userData contains a profileImage field
        } catch (error) {
  ...
  const handleImageUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        localStorage.setItem('profileImage', base64Image); // Store it in localStorage (optional)
      };
  ...
  const handleImageDelete = () => {
    ...
    localStorage.removeItem('profileImage'); // Optionally remove it from localStorage
    ...
    <div className='proedit'>
      ...
      {settingsMenuOpen && (
        <div className="settings-menu">
          <button onClick={toggleDarkMode} className="settings-button">Dark Mode</button>
          {userData ? (
        ...
        <div className="profile-container">
          <div className="profile-header">
            ...
            <img
              ...
              onClick={handleImageClick} // Click to open the image menu
            />
