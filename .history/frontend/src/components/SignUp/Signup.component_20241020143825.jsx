...
const StyledForm = styled.form`
  .form-control {
    margin-bottom: 15px;
    background: #F9F6EE	; /* Light transparent background for input */
    color: black;
...
const Signup = (props) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { AME,Email,password, cpassword } = credentials;
    ...
    try {
      const response = await fetch(`${URL}/api/createuser`, {
        ...
        body: JSON.stringify({ AME, Email,password })
      });
