...
const ChatWindow = () => {
  ...
  useEffect(() => {
    const fetchLatestId = async () => {
      try {
        const response = await fetch('http://localhost:5000/logs/latestId', {
          headers: {
  ...
  const handleFinalSubmit = async () => {
    try {
      ...
      const response = await fetch("http://localhost:5000/logs/createLog", {
        ...
        headers: {
          "Content-Type": "application/json",
          "auth-token": localStorage.getItem("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VyIjp7ImlkIjoiNjdkZTg5NzgzYjVkZWI1MjNjNzlhM2U0In0sImlhdCI6MTc0MjYzNzQzMn0.XL_juyg1TyeSFsmfXSzSuevX5Wx4uMc4hk-fsdmfjWktoken"), // Assuming you store the token in localStorage
        },
