...
const Sidebar = () => {
  ...
  const { selectedEntries, setSelectedEntries, handleSelect } = useSelection();
  const { filteredLogs, selectedLogs } = useContext(LogsContext);
  ...
  const handleEntrySelect = (entry, event) => {
    if (!isSelectionMode) return;
    
    // Prevent click event from bubbling up
    event?.stopPropagation();
  
    if (selectedLogs.includes(entry)) {
      const newSelection = selectedLogs.filter(item => item._id !== entry._id);
      handleSelect(newSelection);
    } else if (selectedLogs.length < 7) {
      handleSelect([...selectedLogs, entry]);
    } else {
      toast.warning("You can only select up to 7 entries");
    }
  };
  ...
  return (
    <SidebarContainer>
      ...
      <ButtonGroup>
        <ActionButton onClick={handleExportClick} disabled={isSelectionMode && selectedLogs.length === 0}
  >
          <FaFileExport />
          <span>
            {isSelectionMode 
              ? `Export Selected (${formattedLogs.length}/7)` 
              : 'Export to PDF'}
          </span>
  ...
  <EntryItemWrapper 
    ...
    onClick={() => !isSelectionMode && handleEntryClick(entry._id)
    }
  >
    {isSelectionMode && (
     <Checkbox
     type="checkbox"
     checked={selectedLogs.some(log => log._id === entry._id)}
     onChange={(e) => handleEntrySelect(entry, e)}
