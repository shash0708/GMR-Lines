...
const Sidebar = () => {
  ...
  const { selectedLogs, handleSelect, setSelectedLogs } = useContext(LogsContext);
  ...
  const handleExportClick = async () => {
    if (selectedLogs.length === 0) {
      toast.warn('Please select logs to export');
      return;
    }
    ...
    <SidebarContainer>
      ...
      <ButtonGroup>
        ...
        >
          ...
          <span>
            {isSelectionMode 
              ? `Export Selected (${selectedLogs.length}/7)` 
              : 'Export to PDF'}
          </span>
